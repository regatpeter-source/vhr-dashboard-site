...
app.get('/api/subscriptions/my-subscription', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (subscriptionId && process.env.STRIPE_SECRET_KEY && String(subscriptionId).startsWith('sub_')) {
      try {
...
app.post('/api/apps/:serial/stop', async (req, res) => {
  ...
  try {
    ...
    // IMPORTANT: un stop explicite doit purger l'état "en cours" pour éviter le retour après refresh/scrcpy.
