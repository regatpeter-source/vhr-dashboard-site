...
try {
    $response = Invoke-WebRequest -Uri "$Domain/api/login" `
        ...
        -Body (ConvertTo-Json @{ username = "vhr"; password = "VHR@Render#2025!SecureAdmin789" }) `
        -UseBasicParsing `
        -TimeoutSec 10 `
        -ErrorAction Stop
...
Write-Host "Admin login: vhr / VHR@Render#2025!SecureAdmin789" -ForegroundColor Yellow
