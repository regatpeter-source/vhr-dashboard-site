...
if (Test-Path $dashboardFile) {
    ...
    if ($content -match "id='battery_.*?d\.serial" -and $content -match "-- --") {
    ...
    if ($tableIndex -gt 0) {
        $tableSection = $content.Substring($tableIndex, 3000)
        if ($tableSection -match "battery_") {
...
if (Test-Path $serverFile) {
    ...
    if ($content -match "dumpsys.*battery") {
        Write-Host "OK Battery extraction logic found (dumpsys battery)" -ForegroundColor Green
    } else {
...
if (Test-Path $dashboardFile) {
    ...
    if ($content -match "let viewMode\s*=") {
        Write-Host "OK viewMode initialization found" -ForegroundColor Green
        Write-Host "   Default is table - battery should display there" -ForegroundColor Cyan
    } else {
        Write-Host "ERROR viewMode initialization NOT found" -ForegroundColor Red
    }
} else {
    Write-Host "ERROR dashboard-pro.js not found" -ForegroundColor Red
}
